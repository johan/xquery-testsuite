<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="/">
      <html>
         <head>
            <title>XQuery and XPath Full Text 1.0 Interoperability Test Suite</title>
         </head>
         <body>
            <xsl:if test="test:test-suite/@targetLanguage">
               <h1><xsl:value-of select="test:test-suite/@targetLanguage" /> XQuery and XPath Full Text 1.0 Interoperability Test Suite</h1>
            </xsl:if>
            <xsl:if test="not( test:test-suite/@targetLanguage )">
               <h1>XPath/XQuery/XQueryX Full Text Interoperability Test Suite</h1>
            </xsl:if>
            ...
            <h2>Introduction</h2>
            <P>
               The following document is an XSLT transform of the XQuery and XPath Full Text 1.0 test catalog. The test 
               catalog is an XML file containing meta-data about the XQuery interoperability 
               tests published by the FTTF. The meta-data serves to link test queries with 
